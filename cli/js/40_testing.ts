...
// TODO improve type
export type TestFunction = (...args: any[]) => any;

// TODO improve type
export type BenchFunction = (...args: any[]) => any;
 ...
 * }} BenchDescription
 */
export interface BenchDescription {
  id: number;
  name: string;
  fn: BenchFunction;
  origin: string;
...
async function runBench(desc) {
  ...
  try {
    ...
    if (desc.sanitizeExit) setExitHandler(null);
...
export async function runBenchmarks() {
  ...
  let groups: Set<unknown> | unknown[] = new Set();
  ...
  ArrayPrototypeSort(
    filtered,
    (a, b) => (groups as unknown[]).indexOf(a.group) - (groups as unknown[]).indexOf(b.group),
  );
...
/** @param desc {TestDescription | TestStepDescription} */
function createTestContext(desc: TestDescription | TestStepDescription) {
  let parent;
  let level;
  let rootId;
  let rootName;
  ...
  return {
    [SymbolToStringTag]: "Deno.TestContext",
    /**
 ...
 * @returns {T}
 */
function wrapTestFnWithSanitizers<T = Function>(testFn: T, opts: {
    sanitizeOps: boolean;
    sanitizeResources: boolean;
    sanitizeExit: boolean;
  }): T {
