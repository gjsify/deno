...
import { ERR_INVALID_MODULE_SPECIFIER, ERR_MODULE_NOT_FOUND } from './errors.js';
...
function tryPackage(requestPath, exts, isMain, originalPath) {
  ...
  if (actual === false) {
    ...
    if (!actual) {
      ...
      ) as Error & { code: string; path: string; requestPath: string; };
...
function finalizeEsmResolution(
  ...
  throw new ERR_MODULE_NOT_FOUND(
    ...
    pathResolve(pkgPath, "package.json"),
  );
}
...
Module._resolveFilename = function (
  ...
  // eslint-disable-next-line no-restricted-syntax
  const err = new Error(message) as Error & { code: string, requireStack: string[]};
  err.code = "MODULE_NOT_FOUND";
...
function packageSpecifierSubPath(specifier) {
  let parts = StringPrototypeSplit(specifier, "/" as any);
